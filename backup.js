...
async function submitJob(
  ...
  const staleAttachments = await initialComposer.composer
    .locator('img,video,[data-slate-void="true"]')
    .count();
  const reference = job.references.find(Boolean);
  if (staleAttachments > 0) {
    throw new Error(
      `${job.code}: composer còn ${staleAttachments} reference từ lần trước. Runner chặn để không dùng nhầm ảnh.`,
    );
  }
  if (reference) await attachReference(page, reference, emit);
